...
/**
  ...
  * @retval 无
  */
void Send_JustFloat(uint8_t val)
{
    Frame frame_data;
    frame_data.fdata[0]=val;
    //拷贝帧尾
    memcpy(frame_data.tail, FRAME_TAIL, 4);
    HAL_UART_Transmit(&huart1, (uint8_t *)&frame_data, sizeof(Frame), 10);
}
/* USER CODE END 0 */
...
int main(void)
{
  ...
  while (1)
  {
    ...
    if(mycan_ReceiveFlag()>0)
    {
      ...
      Send_JustFloat(RxData[0]);
    }

    if(Key_Get()==1)
    {
      Keynum=Keynum+1;
      mycan_SendData(TxID,Txlength,TxData);
    }
    OLED_ShowNum(1,14,Keynum,2);
  }
  /* USER CODE END 3 */
}
